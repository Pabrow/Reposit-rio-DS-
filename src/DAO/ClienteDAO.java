...
public class ClienteDAO {
    ...
    public void editarPorID(Cliente p){
        con = ConexaoSQL.conectar();
        String sql = "update cliente set nome_cli =?, cpf_cli =?, sexo_cli =?, rg_cli =?, tel_cli =?, email_cli = ?, end_cli = ?, dataNasc_cli =? where id_cli=?;";
         try( PreparedStatement stm =con.prepareStatement(sql)){   
            stm.setString(1, p.getNome());
            stm.setString(2, p.getCpf());
            stm.setString(3, p.getSexo());
    ...
    public List<Cliente> listarTodos() {
        ...
        try (PreparedStatement stm =con.prepareStatement(sql)){
            ...
            while(Resultado.next()){
                ...
                p.setId_cliente(Resultado.getInt("id_cli"));

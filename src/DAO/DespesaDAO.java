...
public class DespesaDAO {
    ...
    public void editarPorID(Despesa p){
        con = ConexaoSQL.conectar();
        String sql = "update despesa set desc_desp =?, valor_desp =?, data_desp =?, formaPag_desp =? where id_desp=?;";
         try( PreparedStatement stm =con.prepareStatement(sql)){   
            stm.setString(1, p.getDesc());
            stm.setDouble(2, p.getValor());
            stm.setString(3, p.getData());
    ...
    public void deletar(int id){
        con = ConexaoSQL.conectar();
        String sql = "delete from despesa where id_desp=?";
    ...
    public List<Despesa> listarTodos() {
        ...
        try (PreparedStatement stm =con.prepareStatement(sql)){
            ...
            while(Resultado.next()){
                ...
                p.setId_despesa(Resultado.getInt("id_desp"));
